...
data_folder = 'impro01' # Change only this folder name to iterate over all the 18 audioinfluencer.mid files in each impro folder
midi_folder = f'data/{data_folder}'

# List to store entropy values
entropy_values = []

# Loop through MIDI files with specific names
for i in range(1, 19):
    ...
    result_directory = os.path.join('results', f'{data_folder}', 'entropy', subfolder_name)
    ...
    entropy_csv_path = os.path.join('results', 'impro01', 'entropy', 'entropy_values_all.csv')
    with open(entropy_csv_path, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['File Name', 'Pitch Entropy', 'Intensity Entropy', 'Duration Entropy'])
