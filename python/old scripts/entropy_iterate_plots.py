...
def extract_features_from_midi(midi_path):
    ...
    return entropy_pitch, entropy_intensity, entropy_duration, pitch_values, intensity_values
...
for folder_num in range(1, num_folders+1):
    ...
    for file_num in range(1, num_files_per_folder+1):
        midi_file_path = os.path.join('data', folder_name, f'impro{folder_num:02d}-{file_num:02d}.mid')
        
        # Calculate feature entropies
        entropy_pitch, entropy_intensity, entropy_duration, pitch_values, intensity_values = extract_features_from_midi(midi_file_path)
        ...
        plt.subplot(1, 2, 1)
        plt.hist(pitch_values, bins=range(128), edgecolor='black')
        ...
        plt.subplot(1, 2, 2)
        plt.hist(intensity_values, bins=range(128), edgecolor='black')
        plt.xlabel('MIDI Velocity Value')
        plt.ylabel('Frequency')
        plt.title('Intensity Frequency Distribution')

        # Save the plots
        plot_file_path = os.path.join('results', folder_name, f'impro{folder_num:02d}-{file_num:02d}_frequency_distribution.png')
        plt.tight_layout()
    ...
    print(f'Entropy values and plots for folder {folder_name} saved to {result_file_path}.')
