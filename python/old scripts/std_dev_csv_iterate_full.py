...
def find_pitch_standard_deviation(pitches):
...
for folder_num in range(1, num_folders+1):
    ...
    for file_num in range(1, num_files_per_folder+1):
        ...
        pitch_standard_deviation = find_pitch_standard_deviation(folder_pitches)

        results.append([file_name, pitch_standard_deviation])
    ...
    result_file_path = os.path.join(result_directory, 'standard_deviations.csv')
    with open(result_file_path, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['File Name', 'Pitch Standard Deviation'])
