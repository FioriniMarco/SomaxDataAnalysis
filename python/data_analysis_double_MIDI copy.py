...
# Load the CSV files
df1 = pd.read_csv('data/influencer2.csv')
df2 = pd.read_csv('data/player2.csv')
...
# Apply 1-second break for dataset 1
time_diff = df1.index.to_series().diff().dt.total_seconds()
for event_time in time_diff[time_diff > 1].index:
    axs[0].axvline(x=event_time, color='white', linestyle='--', alpha=0)
